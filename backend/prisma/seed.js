...
async function main() {
  ...
  // Users: 1 admin, 1 teacher login, 1 student login — separate password per role.
  const adminPassword = await bcrypt.hash("anits7890", 10);
  const teacherPassword = await bcrypt.hash("teacher4321", 10);
  const studentPassword = await bcrypt.hash("student4321", 10);

  await prisma.user.upsert({
    where: { email: "admin@college.edu" },
    update: { password: adminPassword },
    create: { name: "Admin", email: "admin@college.edu", password: adminPassword, role: "ADMIN" },
  });

  await prisma.user.upsert({
    where: { email: "kumar@college.edu" },
    update: { password: teacherPassword },
    create: { name: "Mr. Kumar", email: "kumar@college.edu", password: teacherPassword, role: "TEACHER", teacherId: kumar.id },
  });

  await prisma.user.upsert({
    where: { email: "student@college.edu" },
    update: { password: studentPassword },
    create: { name: "Student (CSE-A)", email: "student@college.edu", password: studentPassword, role: "STUDENT", sectionId: cseA.id },
  });

  console.log("Seed complete. Passwords set per role (not printed here — keep them private).");
}
