...
// Manually place a new class into a specific empty slot — used for hand-
// arranging fixed periods before running the smart generator, which then
// treats these (if locked) exactly like it treats any other locked cell:
// never touched, worked around.
router.post("/entry", authenticate, authorize("ADMIN"), async (req, res) => {
  try {
    const { assignmentId, dayId, periodId, classroomId, locked } = req.body;
    const assignment = await prisma.assignment.findUnique({ where: { id: Number(assignmentId) } });
    if (!assignment) return res.status(404).json({ message: "Assignment not found" });

    const teacherIds = [assignment.teacherId, ...(assignment.coTeacherIds || [])];

    // Same conflict checks the generator itself uses: section free, every
    // teacher free, and no "busy elsewhere" clash.
    const [sectionClash, teacherClashes, busySlots] = await Promise.all([
      prisma.timetableEntry.findFirst({ where: { sectionId: assignment.sectionId, dayId: Number(dayId), periodId: Number(periodId) } }),
      prisma.timetableEntry.findMany({
        where: { dayId: Number(dayId), periodId: Number(periodId), OR: teacherIds.map((tid) => ({ teacherId: tid })).concat(teacherIds.map((tid) => ({ coTeacherIds: { has: tid } }))) },
        include: { teacher: true, section: true },
      }),
      prisma.teacherBusySlot.findMany({ where: { dayId: Number(dayId), periodId: Number(periodId), teacherId: { in: teacherIds } } }),
    ]);

    if (sectionClash) {
      return res.status(409).json({ message: "This section already has a class in that slot." });
    }
    if (teacherClashes.length > 0) {
      const c = teacherClashes[0];
      return res.status(409).json({ message: `${c.teacher.name} already teaches ${c.section.name} at that time.` });
    }
    if (busySlots.length > 0) {
      return res.status(409).json({ message: `A teacher on this assignment is marked busy elsewhere at that time${busySlots[0].note ? ` (${busySlots[0].note})` : ""}.` });
    }
    ...
    const entry = await prisma.timetableEntry.create({
      data: {
        ...
        locked: locked !== undefined ? !!locked : true, // locked by default so Generate never overwrites a hand-placed class
      },
      include: entryInclude,
    });
    res.json(entry);
