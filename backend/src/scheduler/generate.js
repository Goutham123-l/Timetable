...
async function generateTimetable() {
  ...
  // Report which sections ended up with unfilled ("Free") slots, so it's
  // clear this came from the Assignment Table not covering every available
  // period — not something the placement step could invent on its own.
  const totalSlotsPerSection = {};
  const sectionLabels = {};
  for (const a of assignments) {
    sectionLabels[a.sectionId] = `${a.section.name}`;
  }
  const placedPerSection = {};
  for (const e of newEntries) {
    placedPerSection[e.sectionId] = (placedPerSection[e.sectionId] || 0) + 1;
  }
  for (const e of lockedEntries) {
    placedPerSection[e.sectionId] = (placedPerSection[e.sectionId] || 0) + 1;
  }
  ...
  const sectionFillSummary = uniqueSectionIds.map((sectionId) => {
    ...
    // Exact list of which (day, period) slots are still empty for this
    // section — not just a count, so it's clear precisely where to add
    // a subject to close the gap.
    ...
    return {
      sectionId,
      section: sectionLabels[sectionId],
      totalSlots,
      placed,
      freeSlots: totalSlots - placed,
      freeSlotDetails,
    };
  });
  const sectionsWithGaps = sectionFillSummary.filter((s) => s.freeSlots > 0);
