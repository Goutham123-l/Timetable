...
const MAX_ATTEMPTS = 25; // internally retried in a single click — see runGeneration()
...
async function generateTimetable(triggeredBy) {
  ...
  function runAttempt() {
    ...
    // Placement order matters: the most constrained items should get first
    // pick of open slots, before flexible theory subjects fill up the grid
    // and leave nothing for a tightly-constrained lab (e.g. one needing 3
    // teachers free together for 2 consecutive periods). So: always-last
    // subjects first (only one slot/day fits them), then labs — hardest
    // labs (most co-teachers) first — then theory subjects last, since
    // theory can go in literally any single free period.
    const alwaysLastAssignments = assignments.filter((a) => a.subject.alwaysLastPeriod);
    const labAssignments = assignments
      .filter((a) => !a.subject.alwaysLastPeriod && a.subject.type === "LAB")
      .sort((a, b) => (b.coTeacherIds?.length || 0) - (a.coTeacherIds?.length || 0));
    const theoryAssignments = assignments.filter((a) => !a.subject.alwaysLastPeriod && a.subject.type !== "LAB");
    ...
    // Labs are tried in constraint order (most co-teachers first), not
    // shuffled, so the hardest ones consistently get first pick every attempt.
    ...
    for (const assignment of shuffle(theoryAssignments)) {
      const remaining = placeAssignment(assignment);
      if (remaining > 0) {
        conflicts.push({
          type: "INSUFFICIENT_SLOTS", teacher: assignment.teacher.name, subject: assignment.subject.name, section: assignment.section.name,
