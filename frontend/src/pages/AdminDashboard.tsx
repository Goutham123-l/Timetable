...
export default function AdminDashboard() {
  ...
  async function loadDashboard() {
    ...
    setLoadError("");
    try {
      // Every call is individually guarded so one failing endpoint (e.g. a
      // brand-new table not yet migrated) can't blank out the whole
      // dashboard — it just shows an empty state for that one piece.
      ...
      // Only reachable if the token itself is invalid (api.ts already
      // redirects to /login in that case) or a genuine network failure.
      setLoadError(e.message || "Could not reach the server. Check your connection and try again.");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loadError) {
    return (
      <div>
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">Dashboard</h2>
        <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-300 rounded-xl p-5 text-sm">
          <p className="font-medium mb-2">Couldn't load dashboard data.</p>
          <p className="mb-3">{loadError}</p>
          <button onClick={loadDashboard} className="bg-red-600 text-white text-xs px-4 py-2 rounded-lg">
            Retry
          </button>
        </div>
      </div>
