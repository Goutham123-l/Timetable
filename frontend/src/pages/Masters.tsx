...
function Card({ children }: { children: React.ReactNode }) {
  return <div className="bg-white dark:bg-slate-800 dark:border-slate-700 rounded-xl border border-slate-200 p-5 shadow-sm transition-colors">{children}</div>;
}
...
function Subjects() {
  ...
  async function deleteAll() {
    ...
    toast.success("Deleted all subjects.");
    load();
  }
    ...
    <Card>
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          {items.length > 0 && <UndoDeleteButton label="all subjects" onConfirm={deleteAll} size="sm" />}
          <button onClick={() => window.print()} className="text-xs bg-slate-700 text-white px-3 py-1.5 rounded-lg">Print</button>
        </div>
      </div>
...
function Teachers() {
  ...
  async function deleteAll() {
    ...
    toast.success("Deleted all teachers, and everything they were scheduled to teach.");
    load();
  }
  ...
  return (
    <Card>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          {items.length > 0 && <UndoDeleteButton label="all teachers" onConfirm={deleteAll} size="sm" />}
          <button onClick={() => window.print()} className="text-xs bg-slate-700 text-white px-3 py-1.5 rounded-lg h-fit">Print</button>
        </div>
      </div>
      <div className="flex gap-2 flex-wrap items-center mb-4">
                        ...
                        <tbody>
                          {assignmentsMap[t.id].map((a: any) => (
                            <tr key={a.id}>
                              ...
                              <Td>
                                <button onClick={() => removeAssignment(t.id, a.id)} className="text-red-500 text-xs font-medium">
                                  Delete
                                </button>
                              </Td>
                            </tr>
